...
if [ "${FLASHBACK_DEMO:-false}" = "true" ]; then
    log "DEMO MODE: Simulating V\$RESTORE_POINT query."
    log ""

    # Simulated restore point data (pipe-delimited for parsing)
    # Format: NAME|TIME|GUARANTEED|STORAGE_SIZE|PDB_RESTORE_POINT|CON_ID
    DEMO_DATA="${INSTANCE_ID}_CDB_26MARCH2026_DATA|26-MAR-26 06.31.28 PM|YES|2.4484E+12|NO|0
...
${INSTANCE_ID}_PDB_flashback_restore_11FEB26|11-FEB-26 14.00.00 PM|YES|2.6322E+11|YES|4"
    ...
    log "=============================================================================="
    log " AVAILABLE RESTORE POINTS (DEMO)"
    log "=============================================================================="
    log ""
    ...
    i=1
    echo "$DEMO_DATA" | while IFS='|' read -r name time gua storage pdb conid; do
    ...
    TOTAL=$(echo "$DEMO_DATA" | wc -l | tr -d ' ')
    ...
    SELECTED_ROW=$(echo "$DEMO_DATA" | sed -n "${selection}p")
